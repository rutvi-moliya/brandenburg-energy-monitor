...
def generate_daily_summary() -> None:
    ...
    try:
        ...
        logger.info("Daily AI summary completed successfully.")
    except Exception as e:
        logger.error(f"generate_daily_summary() failed: {e}", exc_info=True)

# Startup summary check — runs in a background thread after scheduler starts
def _check_and_generate_startup_summary() -> None:
    """
    Runs in a separate thread 5 seconds after startup.
    Checks if today's summary exists — if not, generates it immediately.
    The 5 second delay ensures the scheduler is fully running first.

    This is the permanent fix for missing daily summaries when the app
    starts after 08:00. Runs every single time the app starts.
    ...
    try:
        ...
        if latest is None or latest["summary_date"] != today:
            logger.info(
                f"No summary found for today ({today}). "
                "Generating now..."
            )
            create_and_save_summary()
            logger.info("Startup summary generated successfully.")
        else:
            logger.info(f"Summary for today ({today}) already exists. Skipping.")

    except Exception as e:
        logger.error(f"Startup summary check failed: {e}", exc_info=True)
...
def start_scheduler() -> BackgroundScheduler:
    ...
    Also starts a one-time background thread that checks if today's
    summary exists and generates it if missing. This runs every startup
    so the dashboard always shows a current summary.
    ...
    # Job 2 - daily AI summary at configured hour
    _scheduler.add_job(
    ...
    # Start startup summary check in a separate daemon thread
    # This runs AFTER _scheduler.start() so everything is ready
    # daemon=True means this thread dies automatically when the app exits
    startup_thread = threading.Thread(
        target=_check_and_generate_startup_summary,
        daemon=True,
        name="startup-summary-check"
    )
    startup_thread.start()
    logger.info("Startup summary check thread started.")
